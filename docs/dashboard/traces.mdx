...
Filter by agent, model, workflow, customer, or a metadata key/value. Once an
agent is picked, the **+ Filter** menu also offers **Prompt version**, which
narrows the list to the runs that used one of that agent's inferred
