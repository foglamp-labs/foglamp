...
Filter by agent, model, workflow, customer, or a metadata key/value. The
**+ Filter** menu also offers **Prompt version**, which narrows the list to
the runs that used one inferred
[prompt version](/dashboard/agents#prompt-versions). With an agent picked it
lists that agent's versions; otherwise every agent's, and choosing one also
selects its agent.
