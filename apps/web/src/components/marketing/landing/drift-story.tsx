              ...
              A touch of right padding on mobile keeps the cards off the edge. */}
					<div className="flex flex-col gap-4">
						{PINGS.map((p, i) => (
							<motion.div
								key={p.name}
								className="flex items-start gap-3 rounded-xl squircle:rounded-[36px] corner-squircle dark:shadow-(--custom-shadow) bg-muted/50 p-3.5 shadow-sm backdrop-blur-sm"
								{...reveal(2.4 + i * 0.4, { x: 4 })}
							>
								<span
									className={cn(
										"flex size-9 shrink-0 items-center justify-center rounded-lg squircle:rounded-3xl corner-squircle dark:shadow-(--custom-shadow)",
										p.badge,
									)}
								>
