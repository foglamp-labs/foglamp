	...
	return (
		<>
			...
			<div className="mt-1 flex flex-wrap items-center gap-2 text-xs px-7">
				...
				<DemoContextChip
					icon={(p) => (
						<AgentIcon name={session.agentName} filled className={p.className} />
					)}
				...
				<div className="flex flex-wrap items-center gap-1.5 pl-9">
					{turn.toolCalls.map((tc) => (
						<button
							...
							className={cn(
								"inline-flex cursor-pointer items-center gap-1 rounded-full border bg-card/40 px-2 py-0.5 text-xs text-muted-foreground transition-colors hover:text-foreground",
								tc.errorCount > 0 &&
						...
						>
							...
							{tc.count > 1 && <span className="tabular-nums">×{tc.count}</span>}
						</button>
					))}
				</div>
			)}
