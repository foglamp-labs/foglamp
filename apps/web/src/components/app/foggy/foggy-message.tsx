			...
			{message.parts.map((part, i) => {
				...
				if (part.type === "text") {
					...
					return isUser ? (
						<div
							key={key}
							className="max-w-[85%] whitespace-pre-wrap rounded-lg squircle:rounded-3xl corner-squircle dark:bg-muted bg-muted-foreground/8 px-3 py-2 text-sm text-primary"
						>
							{part.text}
						</div>
					) : (
